...
def compute_Aw_main(A,ep,tpd,tpp,pds,pdp,pps,ppp,tNiOs,tOsOs,tOsOs_p,Upp,\
    ...
    if pam.set_tNiOs_from=='no-eh':
        T_NiOs = ham.create_tNiOs_nn_matrix_from_no_eh (VS,tNiOs_nn_hop_dir, tNiOs_orbs, tNiOs_nn_hop_fac)
    elif pam.set_tNiOs_from=='one-eh':
        T_NiOs = ham.create_tNiOs_nn_matrix_from_one_eh(VS,tNiOs_nn_hop_dir, tNiOs_orbs, tNiOs_nn_hop_fac)
    ...
    if Norb==8 or Norb==10 or Norb==11 or Norb==12:     
        ...
        if pam.if_compute_Aw==1:
            ...
            # compute d9
            d9_state_indices, d9_state_labels, d9_labels = getstate.get_d9_state_indices(VS)
            fig.compute_Aw(H, VS, w_vals, d9_state_indices, d9_labels, "Aw_d9_", fname)

            # compute d10L
            d10L_state_indices, d10L_state_labels = getstate.get_d10L_state_indices(VS)
            fig.compute_Aw(H, VS, w_vals, d10L_state_indices, d10L_state_labels, "Aw_d10L_", fname)

            # compute d8s for some special states
            d8s_state_indices, d8s_state_labels = getstate.get_d8s_state_indices(VS, d_double, \
                                                                                 S_val, Sz_val, AorB_sym, A)
            fig.compute_Aw(H, VS, w_vals, d8s_state_indices, d8s_state_labels, "Aw_d8s_", fname)

            # compute d8s according to d8's sym
            fig.compute_Aw_d8s_sym(H, VS, d_double, S_val, Sz_val, AorB_sym, A, w_vals, "Aw_d8s_sym_", fname)

            # compute d9Ls
            b1Ls_state_indices, b1Ls_state_labels, a1Ls_state_indices, a1Ls_state_labels \
                        = getstate.get_d9Ls_state_indices(VS, S_val, Sz_val)
            fig.compute_Aw1(H, VS, w_vals, b1Ls_state_indices, b1Ls_state_labels, "Aw_b1Ls_", fname)
            fig.compute_Aw1(H, VS, w_vals, a1Ls_state_indices, a1Ls_state_labels, "Aw_a1Ls_", fname)
...
if __name__ == '__main__': 
    ...
    if pam.if_print_VS_after_basis_change==1:
        basis.print_VS_after_basis_change(VS,d_double,S_val,Sz_val)

    U_d = (U.conjugate()).transpose()
    
    # check if U if unitary
    #util.checkU_unitary(U,U_d)
