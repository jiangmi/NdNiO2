...
def compute_Aw(H, VS, w_vals, state_index, label_index, fig_name, fname):
    ...
    for j in range(0,Nstate):
        ...
        plt.plot(w_vals, Aw, Ms[j], linewidth=1, label=label_index[j])  

        # write data into file for reusage
        if pam.if_write_Aw==1:
            util.write_Aw(fig_name+fname+'_'+label_index[j]+'.txt', Aw, w_vals)
...
def compute_Aw_d8s_sym(H, VS, d_double, S_val, Sz_val, AorB_sym, A, w_vals, fig_name, fname):
    ...
    for i in range(0,Nsym):
        ...
        d8s_state_indices = getstate.get_d8s_state_indices_sym(VS,sym,d_double,S_val, Sz_val, AorB_sym, A)

        Aw_dd = np.zeros(len(w_vals))
        for index in d8s_state_indices:      
            Aw, w_peak, weight = getAw(H,index,VS,w_vals)
        ...
        if pam.if_write_Aw==1:
            ...
            util.write_Aw(fig_name+fname+'_'+sym+'.txt', Aw_dd, w_vals)

        # accumulate Aw for each sym into total Aw_dd
        Aw_dd_total += Aw_dd

        subplot(Nsym,1,i+1)
        plt.plot(w_vals, Aw_dd, Ms[i], linewidth=1, label=sym+'s')
    ...
    if Nsym>0 and pam.if_savefig_Aw==1:    
        savefig(fig_name+fname+"_sym.pdf")
